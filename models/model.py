...
def rxt(A_bank, leverage_borrower, sigma, theta):
    ...
    l_safe = np.maximum(leverage_borrower, 1e-6) 

    base_rate = sigma * (max(A_z_safe, 1e-6) ** (-sigma))
    risk_premium = theta * (l_safe ** theta)
    return base_rate + risk_premium

def Bxt(wage_bill_x, A_vector):
    """"
    ...
    """
    return np.maximum(wage_bill_x - A_vector, 0)
...
class Economy:
    ...
    def __init__(self, params, supplier=None, bank_links_d=None, bank_links_u=None):
        ...
        self.history = {
            ...
            "count_defaults_u": [],
            "count_defaults_z": [],
            "Q_mismatch_d": [], #diff entre (oferta e demanda) -- DESCONTINUADO
        }
    ...
    def step_profits_and_dynamics(self, Y_d, revenue_d, Q_d_demand, price_intermediate, B_d, 
        ...
        #pagamento por bens intermediarios Downstream -> Upstream
        ...
        for i in range(p.N_d):
            ...
            A_d_pre_default[i] = (revenue_d[i] - bank_repayment_d[i] - cost_trade_d[i]) + equity_remanescente
        ...
        bank_repayment_u = (1 + r_bank_u) * B_u #juros

        # receita e atualizacao Upstream
        for j in range(p.N_u):
            equity_remanescente = max(self.A_u[j] - wage_bill_u[j], 0) #pagamento de custos
            A_u_pre_default[j] = (revenue_u[j] - bank_repayment_u[j]) + equity_remanescente
